    ...
    if(x<width && y<height && k<len)
    {
        ...
        for(int b=0;b<batch;b++)
        {
            for(int ch=0;ch<channel;ch++)
            {
                ...
                grad_weight[((b*len+k)*sp+y*width+x)] += point_feature*dout;
            }
        }
    }
...
__global__ void mapping_with_weight_backward_feature_kernel(const T* grad_out, const T* weight,T* grad_feature,const int batch,const int channel,const int height,const int width)
{
    ...
    if(x<width && y<height && ch<channel)
    {
         ...
         for(int b=0;b<batch;b++)
         {
             ...
             {
                if(k<height)
                    grad_feature[(b*channel+ch)*sp+y*width+x] += weight[((b*len+y)*sp+k*width+x)]*grad_out[(b*channel+ch)*sp+k*width+x];
                else
                {
                    int cord = k -height;
                    if(cord<x)
                        grad_feature[(b*channel+ch)*sp+y*width+x] += weight[((b*len+x+height-1)*sp+y*width+cord)]*grad_out[(b*channel+ch)*sp+y*width+cord];
                    else
                        grad_feature[(b*channel+ch)*sp+y*width+x] += weight[((b*len+x+height)*sp+y*width+cord+1)]*grad_out[(b*channel+ch)*sp+y*width+cord+1];
                }
...
at::Tensor mapping_with_weight_forward(const at::Tensor weight, const at::Tensor feature)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(out.type(),"mapping forward cuda",([&]{
        mapping_with_weight_forward_kernel<scalar_t><<<blocks,threads>>>(
